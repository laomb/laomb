...
def lowercase_regs_safe(s: str) -> str:
    def fn(t):
        return REGS_RX.sub(lambda m: m.group(0).lower(), t)
...
def format_instruction(mnemonic: str, rest: str) -> str:
    ...
    m = (
        mnemonic
        if (low.endswith("*") and not any(low.startswith(x) for x in ("set", "cmov")))
        else low
    )
    body = rest.strip()
    if not body:
...
class Context:
    ...
    def total_depth(self):
        return self.block_depth + self.brace_depth
...
def normalize_line(code: str, comment: str, indent: int, ctx: Context) -> str:
    ...
    if low_mnem in BLOCK_OPEN or low_mnem.startswith(
        ("if", "match", "irp", "irps", "iterate")
    ):
        ctx.block_depth += 1
    if low_mnem in BLOCK_CLOSE or low_mnem.startswith("endif"):
