...
static void setup_fasmg_env(void) {
	...
	const char *loom_inc_root = cb_join(L->LOOM_DIR, "include");

	const char *old = getenv("INCLUDE");

	char merged[8192];
	if (old && *old) {
		snprintf(merged, sizeof(merged), "%s;%s;%s", global_inc_root, loom_inc_root, old);
	} else {
		snprintf(merged, sizeof(merged), "%s;%s", global_inc_root, loom_inc_root);
	}
...
static int cmd_default(void) {
	const layout_t *L = (const layout_t *)cb_shared_current();
	if (!L) {
		cb_log_error("no shared layout handed to loom");
		return 2;
	}
	setup_fasmg_env();

	const char *src = cb_join(L->LOOM_DIR, "loom.asm");
	if (!cb_file_exists(src)) {
		cb_log_error("missing %s", src);
		return 2;
	}

	int rc = build_one(L, src, L->LOOM_BIN);
	if (rc)
		return rc;

	cb_log_info("loom build complete: %s", L->LOOM_BIN);
	return 0;
}
...
CB_API void carbide_recipe_main(cb_context *ctx) {
	...
	cb_register_cmd("build", cmd_default, "assemble loom.asm into loom.bin");
